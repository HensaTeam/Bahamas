...
function cRP.initPrison(nuser_id,services,fines,text)
	...
	if user_id then
		if actived[user_id] == nil then
			...
			vRP.query("characters/setPrison",{ user_id = parseInt(nuser_id), prison = parseInt(services) })
...
function cRP.updatePort(nuser_id)
	...
	if user_id then
		...
		if nidentity["port"] == 0 then
			portStatus = "Ativado"
			vRP.query("characters/updatePort",{ port = 1, id = parseInt(nuser_id) })
		else
			portStatus = "Desativado"
			vRP.query("characters/updatePort",{ port = 0, id = parseInt(nuser_id) })
		end
		...
		if vRP.hasPermission(user_id,"servicePolice") then
			...
			if identity then
				if identity["penal"] == 0 then
					vRP.query("characters/updatePenal",{ penal = 1, id = parseInt(nuser_id) })
				else
					vRP.query("characters/updatePenal",{ penal = 0, id = parseInt(nuser_id) })
				end
...
function cRP.resetPrison()
	...
	if user_id then
		...
		vRP.query("characters/fixPrison",{ user_id = parseInt(user_id) })
...
function cRP.reducePrison()
	...
	local user_id = vRP.getUserId(source)
	if user_id then
		vRP.query("characters/removePrison",{ user_id = parseInt(user_id), prison = 5 })
