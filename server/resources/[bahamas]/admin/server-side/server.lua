...
RegisterCommand("gemstone",function(source,args,rawCommand)
    ...
    if user_id and parseInt(args[1]) > 0 and parseInt(args[2]) > 0 then
        if vRP.hasGroup(user_id,"Moderador") then
            ...
            if identity then
                vRP.query("accounts/infosUpdategems",{ license = identity["license"], gems = gems })
...
RegisterCommand("addbank",function(source,args,rawCommand)
    ...
    if user_id and parseInt(args[1]) > 0 and parseInt(args[2]) > 0 then
        if vRP.hasGroup(user_id,"Moderador")then
			...
			vRP.query("characters/addBank",{ id = nuser_id, bank = bank })
...
RegisterCommand("priority",function(source,args,rawCommand)
	...
	if user_id and parseInt(args[1]) > 0 then
		if vRP.hasGroup(user_id,"Moderador") then
			...
			local identity = vRP.userIdentity(nuser_id)
			if identity then
				vRP.query("accounts/setPriority",{ license = identity["license"], priority = 99 })
			end
		end
	end
end)
...
RegisterCommand("delete",function(source,args,rawCommand)
	...
	if user_id and args[1] then
		if vRP.hasGroup(user_id,"Moderador") then
			local nuser_id = parseInt(args[1])
			vRP.query("characters/removeCharacters",{ id = nuser_id })
	...
	if user_id then
		if vRP.hasGroup(user_id,"Suporte") and parseInt(args[1]) > 0 and parseInt(args[2]) > 0 then
			...
			if identity then
				...
				vRP.query("banneds/insertBanned",{ license = identity["license"], days = days })
