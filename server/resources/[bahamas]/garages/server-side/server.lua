				...
				elseif parseInt(os.time()) <= parseInt(vehicle[1]["time"] + 24 * 60 * 60) then
					...
					if status then
						if vRP.paymentFull(user_id,vehPrice * 0.5) then
							vRP.query("vehicles/arrestVehicles",{ user_id = parseInt(user_id), vehicle = vehName, arrest = 0, time = 0 })
						else
...
function cRP.tryDelete(vehNet,vehEngine,vehBody,vehFuel,vehDoors,vehWindows,vehTyres,vehPlate,vehBrake)
	if vehList[vehNet] and vehNet ~= 0 then
		...
		if vehicle[1] ~= nil then
			vRP.query("vehicles/updateVehicles",{ user_id = parseInt(user_id), vehicle = tostring(vehName), engine = parseInt(vehEngine), body = parseInt(vehBody), fuel = parseInt(vehFuel), doors = json.encode(vehDoors), windows = json.encode(vehWindows), tyres = json.encode(vehTyres), brakes = json.encode(vehBrake) })
		end
	end
					...
					if identity then
						...
						if vRP.request(source,"Deseja transferir o veículo <b>"..vehicleName(args[2]).."</b> para <b>"..identity["name"].." "..identity["name2"].."</b>?",30) then
							...
							if vehicle[1] then
								...
								vRP.query("vehicles/moveVehicles",{ user_id = parseInt(user_id), nuser_id = parseInt(nuser_id), vehicle = args[2] })

								local custom = vRP.query("entitydata/getData",{ dkey = "custom:"..user_id..":"..args[2] })
								if parseInt(#custom) > 0 then
									vRP.query("entitydata/setData",{ dkey = "custom:"..nuser_id..":"..args[2], value = custom[1]["dvalue"] })
									vRP.query("entitydata/removeData",{ dkey = "custom:"..user_id..":"..args[2] })
								end
