			...
			if itemType(totalName) == "Usável" or itemType(totalName) == "Animal" then
				...
				if nameItem == "newgarage" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,false,slot) then
						vRP.query("characters/updateGarages",{ id = parseInt(user_id) })
						TriggerClientEvent("inventory:Update",source,"updateMochila")
				...
				if nameItem == "newprops" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,false,slot) then
						vRP.query("characters/updateHomes",{ id = parseInt(user_id) })
						TriggerClientEvent("inventory:Update",source,"updateMochila")
				...
				if nameItem == "namechange" then
					...
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
						vRP.query("characters/updateName",{ name = name, name2 = name2, user_id = parseInt(user_id) })
				...
				if nameItem == "chip" then
					...
					if string.len(initCheck) == 3 and string.len(finiCheck) == 3 then
						if not vRP.userPhone(firstNumber.."-"..lastNumber) then
							if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
								vRP.query("characters/updatePhone",{ phone = firstNumber.."-"..lastNumber, id = parseInt(user_id) })
							...
							repeat
								if activeItens[user_id] == 0 then
									...
									if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
										...
										vRP.query("vehicles/updateHardness",{ vehicle = vehName, plate = vehPlate, hardness = 1 })
									end
								end
					...
					if vehicle[1] then
						...
						if string.len(plateCheck) ~= 8 then
							...
							if vRP.userPlate(namePlate) then
								...
								if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
									vRP.query("vehicles/plateVehiclesUpdate",{ user_id = parseInt(user_id), vehicle = vehModel, plate = string.upper(namePlate) })
					...
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
						...
						if identity then
							if not vRP.userPremium(user_id) then
								vRP.query("accounts/setPremium",{ license = identity["license"], premium = parseInt(os.time()), predays = 3, priority = 30 })
							else
								vRP.query("accounts/updatePremium",{ license = identity["license"], predays = 3 })

								local infoAccount = vRP.infoAccount(identity["license"])
								if infoAccount then
									if parseInt(infoAccount["priority"]) < 30 then
										vRP.query("accounts/setPriority",{ license = identity["license"], priority = 30 })
									end
								end
							end
						end
				...
				if nameItem == "premium02" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
						...
						if identity then
							if not vRP.userPremium(user_id) then
								vRP.query("accounts/setPremium",{ license = identity["license"], premium = parseInt(os.time()), predays = 7, priority = 45 })
							else
								vRP.query("accounts/updatePremium",{ license = identity["license"], predays = 7 })

								local infoAccount = vRP.infoAccount(identity["license"])
								if infoAccount then
									if parseInt(infoAccount["priority"]) < 45 then
										vRP.query("accounts/setPriority",{ license = identity["license"], priority = 45 })
									end
								end
							end
						end
				...
				if nameItem == "premium03" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
						...
						if identity then
							if not vRP.userPremium(user_id) then
								vRP.query("accounts/setPremium",{ license = identity["license"], premium = parseInt(os.time()), predays = 15, priority = 60 })
							else
								vRP.query("accounts/updatePremium",{ license = identity["license"], predays = 15 })

								local infoAccount = vRP.infoAccount(identity["license"])
								if infoAccount then
									if parseInt(infoAccount["priority"]) < 60 then
										vRP.query("accounts/setPriority",{ license = identity["license"], priority = 60 })
									end
								end
							end
						end
				...
				if nameItem == "premium04" then
					if vRP.tryGetInventoryItem(user_id,totalName,1,true,slot) then
						...
						if identity then
							if not vRP.userPremium(user_id) then
								vRP.query("accounts/setPremium",{ license = identity["license"], premium = parseInt(os.time()), predays = 30, priority = 75 })
							else
								vRP.query("accounts/updatePremium",{ license = identity["license"], predays = 30 })

								local infoAccount = vRP.infoAccount(identity["license"])
								if infoAccount then
									if parseInt(infoAccount["priority"]) < 75 then
										vRP.query("accounts/setPriority",{ license = identity["license"], priority = 75 })
									end
								end
							end
						end
...
AddEventHandler("vRP:playerLeave",function(user_id,source)
	if ammoPlayers[user_id] then
		vRP.query("playerdata/setUserdata",{ user_id = parseInt(user_id), key = "Weapons", value = json.encode(ammoPlayers[user_id]) })
